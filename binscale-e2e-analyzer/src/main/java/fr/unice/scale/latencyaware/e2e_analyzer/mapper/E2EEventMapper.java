...
public class E2EEventMapper {
    public static E2EEventTracker toE2EEventTracker(ConsumerRecord<String, EventCustomer> event) {
        E2EEventTracker tracker = new E2EEventTracker(event.key());
        tracker.addEvent(toE2EEvent(event));
        return tracker;
    }
    ...
    public static E2EEvent toE2EEvent(ConsumerRecord<String, EventCustomer> event) {
        ...
        return new E2EEvent(nodeOrigin, Instant.ofEpochMilli(event.timestamp()));
    }
}
